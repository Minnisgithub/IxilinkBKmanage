          ...
          <el-col :span="6">
            <el-form-item label="消息类型:">
              <el-select v-model="formInline.modifyFlag">
                ...
                <el-option
                  label="更新"
                  value="UPDATE"></el-option>
                <el-option
                  label="查询"
                  value="QUERY"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
...
export default {
  ...
  mounted() {
    if (this.$route.query.beginTime) {
      ...
      this.formInline.time= [query.beginTime,query.endTime]
      this.formInline.time[1] = query.endTime
      this.formInline.interfaceNameOrCode = query.interFaceCode
        ? query.interFaceCode
        : ''
      this.formInline.resSysNameOrCode = query.sysCode ? query.sysCode : ''
      if (query.status == 'success') {
        this.formInline.result = '成功'
      } else if (query.status == 'fail') {
        this.formInline.result = '失败'
      } else {
        this.formInline.result = ''
      }
      this.onSubmit(1, 10, false,query?.exception)
    } else {
  ...
  methods: {
    onSubmit(current, size, time = false,exception) {
      if (time) {
